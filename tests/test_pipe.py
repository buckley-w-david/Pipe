...
@given(integers())
def test_simple_equality(x):
    assert Pipe(x) == Pipe(x)
...
class TestLPartial:
    """
    Inspired from the CPython functools test suite
    """

    def test_basic_examples(self):

        p = lpartial(capture, 1, 2, a=10, b=20)
        assert callable(p)

        assert p(3, 4, b=30, c=40) == ((3, 4, 1, 2), dict(a=10, b=30, c=40))

        p = lpartial(map, [1, 2, 3, 4])
        assert list(p(lambda x: x * 10)) == [10, 20, 30, 40]
...
"""
    TODO: Adapt more tests from CPython test suite
